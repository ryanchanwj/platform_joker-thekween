...
#   }
  ...
  # lambda_config {
  #   pre_token_generation = module.lambda_pre_auth.invoke_arn
  # }
}

resource "aws_cognito_user_pool_client" "client" {
  name = "hk-customer-client"
  ...
  callback_urls = ["https://${data.aws_cloudfront_distribution.hk_www.domain_name}"]
  logout_urls = ["https://${data.aws_cloudfront_distribution.hk_www.domain_name}/_identity/logout"]
  supported_identity_providers = ["COGNITO"]
}

resource "aws_cognito_user_pool_domain" "cognito-domain" {
  domain       = "authhk.jokerandthekween.click"
  user_pool_id = "${aws_cognito_user_pool.user_pool.id}"
