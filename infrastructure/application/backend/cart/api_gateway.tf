...
resource "aws_apigatewayv2_integration" "update_cart" {
  ...
  integration_uri    = aws_lambda_function.update_cart.invoke_arn
  integration_type   = "AWS_PROXY"
  integration_method = "POST"
}
...
resource "aws_apigatewayv2_integration" "view_cart" {
  ...
  integration_uri    = aws_lambda_function.view_cart.invoke_arn
  integration_type   = "AWS_PROXY"
  integration_method = "POST"
}
...
resource "aws_apigatewayv2_integration" "delete_from_cart" {
  ...
  integration_uri    = aws_lambda_function.delete_from_cart.invoke_arn
  integration_type   = "AWS_PROXY"
  integration_method = "POST"
}
